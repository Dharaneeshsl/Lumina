...
import { runLeetcodeDailySync } from '../cron/leetcode.daily-sync.job'
import { syncProfileById } from '../features/leetcode/leetcode.sync.service'
import {
...
leetcodeWorker.on('failed', (job, error) => {
  console.error(`[leetcode-worker] Job ${job?.id} (${job?.name}) failed:`, error.message)
})
