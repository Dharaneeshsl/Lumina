...
export function subscribe(userId: string, response: Response) {
  ...
  return () => { set.delete(response); if (set.size === 0) clients.delete(userId) }
}

export function publishNotification(userId: string, payload: unknown) {
  const data = JSON.stringify(payload)
  for (const response of clients.get(userId) ?? []) response.write(`event: notification\ndata: ${data}\n\n`)
}

export function heartbeat() {
  for (const responses of clients.values()) for (const response of responses) response.write(': heartbeat\n\n')
}
