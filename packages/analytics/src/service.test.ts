...
class Repo implements AnalyticsRepository {
  ...
  async anonymizeUser(userId: string) {
    ...
    for (const event of this.events) {
      if (event.actor?.userId === userId) {
        event.actor = { anonymousId: 'deleted-user' }
        affected++
      }
    }
    return affected
  ...
  async deleteUser(userId: string) {
    ...
    this.events = this.events.filter((event) => event.actor?.userId !== userId)
