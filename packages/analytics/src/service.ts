...
export interface AnalyticsRepository {
  ...
  list(input: { from: Date; to: Date; collegeId?: string; names?: AnalyticsEventName[] }): Promise<StoredAnalyticsEvent[]>
...
export function validateEvent(input: AnalyticsEventInput): AnalyticsEventInput {
  if (!input.idempotencyKey || input.idempotencyKey.length > 200) throw new Error('ANALYTICS_IDEMPOTENCY_KEY_REQUIRED')
...
export class AnalyticsService {
  ...
  async ingest(input: AnalyticsEventInput, tenantId: string | null): Promise<{ duplicate: boolean; event?: StoredAnalyticsEvent }> {
    const event = validateEvent(input)
    if ((event.consent as AnalyticsConsent | undefined) === 'DENIED') return { duplicate: false }
  ...
  async dashboard(from: Date, to: Date, collegeId?: string) {
    ...
    return {
      ...
      to: to.toISOString(),
      totals: { events: events.length, activeUsers: users, engagementRate: engagementRate(users, Math.max(users, 1)) },
      eventCounts: counts,
      featureUsage,
      dataQuality: { invalid: 0, duplicateRate: 0, freshnessSeconds: 0, expectedDays: days },
    }
  }

  async privacy(userId: string, action: 'EXPORT' | 'DELETE' | 'ANONYMIZE', from = new Date(0), to = new Date()) {
    if (action === 'EXPORT') return this.repository.list({ from, to }).then((events) => events.filter((event) => event.actor?.userId === userId))
