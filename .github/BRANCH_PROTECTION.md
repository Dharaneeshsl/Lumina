...
These settings are repository settings and must be applied by a GitHub organization owner or repository administrator. Local files cannot enforce them.
...
Also enable automatic deletion of merged head branches. After configuration, use a non-admin account to confirm direct and force pushes to `main` are rejected.
